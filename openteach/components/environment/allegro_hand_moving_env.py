...
class AllegroHandMovingEnv(Hand_Env):
    def __init__(
        ...
        self.table_asset = self.gym.load_urdf(
            self.sim, asset_root, table_asset_file, table_asset_options
        )
        ...
        self.object_asset = self.gym.load_urdf(
    ...
    def create_env(self):
        ...
        object_pos = {
            ...
            "banana": gymapi.Vec3(1.0, 1.3, 0.01),
        ...
        object_handle = self.gym.create_actor(
            self.env, self.object_asset, object_pose, "object", 0, 0, 0
        )
    ...
    def reset(self, object_position, object_rotation):
        ...
        self.root_state_tensor[self.object_indices[0], 0:3] = to_torch(
            np.array([object_position.x, object_position.y, object_position.z]),
            dtype=torch.float,
            device="cpu",
        )
        self.root_state_tensor[self.object_indices[0], 3:7] = to_torch(
            np.array(
                [
                    object_rotation.x,
                    object_rotation.y,
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(self.object_indices),
    ...
    def compute_observation(self, observation):
        ...
        if observation == "image":
            self.state = self.gym.get_camera_image(
    ...
    def get_rgb_depth_images(self):
        ...
        while color_image is None:
            ...
            depth_image = self.gym.get_camera_image_gpu_tensor(
                self.sim, self.env, self.camera_handle, gymapi.IMAGE_DEPTH
            )
    ...
    def take_action(self):
        ...
        self.root_state_tensor[0, 0:7] = torch.tensor(
            self.previous_endeff, dtype=torch.float32, device="cpu"
        )
        self.actor_indices = to_torch(
            self.actor_indices, dtype=torch.int32, device="cpu"
        )
        self.gym.set_actor_root_state_tensor_indexed(
            self.sim,
            gymtorch.unwrap_tensor(self.root_state_tensor),
