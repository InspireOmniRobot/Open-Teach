...
class XelaCurvedSensors(Component):
        ...
        while True:
            try:
                ...
                if not sensor_state is None:
                    (
                        curr_sensor_palm_values,
                        curr_sensor_fingertip_values,
                        curr_sensor_finger_values,
                        timestamp,
                    ) = sensor_state
                    if (
                        curr_sensor_palm_values is None
                        and curr_sensor_finger_values is None
                        and curr_sensor_fingertip_values is None
                    ):
                        self.timer.end_loop()
                        continue
                    ...
                    if (
                        palm_bias_values is not None
                        and finger_bias_values is not None
                        and fingertip_bias_values is not None
                    ):
    ...
    def get_sensor_state(self):
        (
            ...
            curr_sensor_finger_values,
            timestamp,
        ) = self._controller.get_sensor_state()
        if (
            curr_sensor_palm_values is not None
            and curr_sensor_finger_values is not None
            and curr_sensor_fingertip_values is not None
        ):
            normalized_state = dict(
